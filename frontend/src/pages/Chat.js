...
const Chat = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      message.error(`Ошибка при отправке сообщения: ${errorText}`);
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            ...
            <Select
              ...
              style={{ minWidth: 300, flex: '0 0 auto' }}
            ...
            <Text type="secondary" style={{ whiteSpace: 'nowrap' }}>({assistants.length} помощников)</Text>
            <Button 
              icon={<ReloadOutlined />} 
          ...
          {selectedAssistant && (
            <div style={{ 
              ...
              <Tag color="green" icon={<RobotOutlined />} style={{ margin: 0, fontSize: '13px' }}>
                {selectedAssistant.name}
              </Tag>
              <Tag color="blue" size="small" style={{ margin: 0, fontSize: '12px' }}>
                {selectedAssistant.model}
              </Tag>
              {selectedAssistant.temperature && (
                <Tag color="orange" size="small" style={{ margin: 0, fontSize: '12px' }}>
                  T: {selectedAssistant.temperature}
                </Tag>
              )}
              {selectedAssistant.max_tokens && (
                <Tag color="purple" size="small" style={{ margin: 0, fontSize: '12px' }}>
                  Max: {selectedAssistant.max_tokens}
                </Tag>
              )}
            </div>
          )}
